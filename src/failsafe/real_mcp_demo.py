...
from failsafe.razorpay_client import simulate_mode

DATA_PATH = DATA_DIR / "halted_subscriptions.json"
LOG_PATH = LOGS_DIR / "real_mcp_server_run.jsonl"
RESULTS_PATH = PROJECT_ROOT / "REAL_MCP_RESULTS.md"
...
async def run(n: int = 5):
    ...
    print(
        f"Running {len(sample)} records through the REAL Razorpay official MCP server (test mode)...\n"
    )
    ...
    async with Client(mcp_server) as client:
        for sub in sample:
            ...
            print(
                f"{sub['subscription_id']}  {sub['decline_code']:24s} -> {tool_name or '(no tool)'}"
            )
            if tool_result:
