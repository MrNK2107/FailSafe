...
from failsafe.decline_codes import DECLINE_CODES, RecoveryAction, get_decline_code
from failsafe.detect import LEAVE_ALONE, NEEDS_ATTENTION, detect_at_risk
from failsafe.diagnose import diagnose_decline_code
from failsafe.gate import MAX_ATTEMPTS_PER_SUBSCRIPTION, Gate
from failsafe.ollama_client import propose_action
...
async def process_record(
    ...
    *,
    ...
    if run_detection:
        ...
        if inject_failure == "detection_parse_failure":
            ...
            detection = detect_at_risk(
                ...
                days_since_last_successful_charge=record.get(
        ...
        if classification is None:
            ...
            detection_matched_ground_truth = (
                ...
                if ground_truth_needs_attention is not None
                else None
            )
            audit.log(
                "detection_decision",
    ...
    if inject_failure == "unknown_decline_code" or record["decline_code"] not in DECLINE_CODES:
        reason = (
            f"No policy entry for decline_code '{record['decline_code']}' - needs human review."
        )
        audit.log(
    ...
    if raw_signal:
        ...
        if diagnosed_code is None or diagnosed_code not in DECLINE_CODES:
            ...
            tool_result = extract_tool_text(call)
            audit.log(
                "mcp_tool_call", tool="flag_for_manual_review", result=tool_result, **id_kwargs
            )
            return {
                **id_kwargs,
                "amount_paise": record["amount_paise"],
    ...
    if inject_failure == "llm_parse_failure":
        ...
        action_input_record = (
            record
            if effective_code == record["decline_code"]
