...
from failsafe.generate_data import PLANS, RAW_SIGNAL_TEMPLATES
from failsafe.paths import DATA_DIR

OUTPUT_PATH = DATA_DIR / "detection_pool.json"
...
def _make_healthy(
    rng: random.Random, subscription_id: str, merchant_num: int, plan_name: str, amount_paise: int
) -> dict:
...
def _make_at_risk(
    ...
    if is_early:
        ...
        halted_days_ago = (
            days_since_last_successful_charge if subscription_status == "halted" else None
        )
