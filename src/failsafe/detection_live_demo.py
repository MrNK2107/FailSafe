...
SIMULATE is forced True here via failsafe.razorpay_client.force_simulate(),
the single shared simulate knob (see that module's docstring for why the
old two-patch approach was replaced).
...
DATA_PATH = DATA_DIR / "detection_pool.json"
AUDIT_PATH = LOGS_DIR / "detection_demo_audit.jsonl"
RESULTS_PATH = PROJECT_ROOT / "DETECTION_DEMO_RESULTS.md"
...
def write_results(results: list[dict]):
    ...
    false_positives = [
        ...
        and r["detection_classification"] == "needs_recovery_attention"
    ]
    false_negatives = [
        r
        for r in results
    ...
    unattempted_at_risk_value_paise = sum(
        r["amount_paise"] for r in false_negatives if r.get("amount_paise")
    )
    wasted_calls_on_healthy = sum(
        1 for r in false_positives if r["final_action"] != LEFT_ALONE_BY_DETECTION
    )
    ...
    print(
        f"False positives: {len(false_positives)} (wasted real tool calls: {wasted_calls_on_healthy})"
    )
    print(
        f"False negatives: {len(false_negatives)} (Rs {unattempted_at_risk_value_paise / 100:,.2f} never attempted)"
    )
