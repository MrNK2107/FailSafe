...
DIAGNOSIS behavior, not creating real Razorpay objects. Implemented via failsafe.razorpay_client.force_simulate(), the single
shared simulate knob (no more two-patch dance - see that module's
docstring). Direct programmatic use of `process_one()` (e.g. from a
test) is unaffected and still respects whatever simulate state is
active at call time.
"""
...
from failsafe.abandonment_gate import AbandonmentGate
from failsafe.audit_log import AuditLogger
from failsafe.checkout_abandonment_policy import AbandonmentAction, get_abandonment_policy
from failsafe.diagnose_checkout_abandonment import diagnose_abandonment_reason
...
RESULTS_PATH = PROJECT_ROOT / "CHECKOUT_ABANDONMENT_RESULTS.md"

ID_FIELD = "cart_id"

ACTION_TO_DESCRIPTION = {
    AbandonmentAction.IMMEDIATE_PAYMENT_LINK_RESEND: "Complete your payment for {item} - pick up right where you left off.",
    AbandonmentAction.PAYMENT_LINK_ALTERNATE_METHODS_NUDGE: "Complete your payment for {item} - now with more payment methods to choose from.",
    AbandonmentAction.DISCOUNTED_INCENTIVE_NUDGE: "Complete your payment for {item} - here's a little something extra to say thanks for coming back.",
    AbandonmentAction.DELAYED_NUDGE_NO_DISCOUNT: "Still interested? Complete your payment for {item} whenever you're ready.",
}
...
def _valid_reason(reason) -> bool:
    from failsafe.checkout_abandonment_policy import AbandonmentReason
...
async def process_one(
    ...
    if diagnosed_reason is None or not _valid_reason(diagnosed_reason):
        ...
        audit.log(
            "mcp_tool_call", tool="flag_for_manual_review", result=tool_result, cart_id=cart_id
        )
        return {
            "cart_id": cart_id,
...
async def _process_all(
    client: Client, gate: AbandonmentGate, audit: AuditLogger, carts: list[dict]
) -> list[dict]:
    results = []
    for cart in carts:
        try:
