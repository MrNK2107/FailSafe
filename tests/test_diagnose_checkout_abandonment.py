...
import failsafe.diagnose_checkout_abandonment as diag
...
def test_no_tool_call_returns_none_reason():
    with patch(
        "failsafe.diagnose_checkout_abandonment.requests.post",
        return_value=_fake_response({"message": {}}),
    ):
        result = _call()
...
def test_malformed_tool_call_arguments_returns_none_reason():
    ...
    with patch(
        "failsafe.diagnose_checkout_abandonment.requests.post", return_value=_fake_response(body)
    ):
        result = _call()
    assert result["reason"] is None
...
def test_missing_reason_field_returns_none():
    body = {
        "message": {
            "tool_calls": [{"function": {"arguments": {"reasoning": "no reason key at all"}}}]
        }
    }
    with patch(
        "failsafe.diagnose_checkout_abandonment.requests.post", return_value=_fake_response(body)
...
def test_correct_classification_of_an_unambiguous_case():
    ...
    with patch(
        ...
        result = _call(
            checkout_stage="review_confirm", minutes_since_abandonment=4, amount_paise=899900
        )
    ...
    body = {
        "message": {
            "tool_calls": [
                {
                    "function": {
                        "arguments": {"reason": "aliens_intervened", "reasoning": "hallucinated"}
                    }
                }
            ]
        }
...
def test_transient_failure_then_success_retries_and_recovers():
    body = {
        "message": {
            "tool_calls": [
                {"function": {"arguments": {"reason": "otp_delay_or_failure", "reasoning": "ok"}}}
            ]
        }
    }
    with (
        ...
        patch("failsafe.diagnose_checkout_abandonment.time.sleep"),
    ):
